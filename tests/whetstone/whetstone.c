...
float time_in_secs(float ticks);

float get_timer_value(void)
{
...
int Whetstone() // ------------ Metoda -----------
{
    ...
    printf("default %d MHz ...\n", SAAT_FREKANSI / 1000000);
    ...
    long loopstart = 0;
    float startsec,finisec = 0;
    float KIPS;
    int continuous;

    loopstart = LOOPS;       /* 1000 see the note about LOOP below */
    continuous = 0;
...
float time_in_secs(float ticks)
{
  // scale timer down to avoid uint64_t -> float conversion in RV32
  int scale = 256;
  float delta = ticks / scale;
  float freq = SAAT_FREKANSI / scale;
  return delta / freq;
}
