...
main(int argc, char *argv[])
{
    ...
    uart_aktiflestir();
    int rtrd = retired_oku();
    printf("%d\n", rtrd);
    /*while((1==1)){
        printf("a");
        //printf("%d", time_in_secs(get_time()));
        printf("\r\n");
    }*/
...
#error "Please define a way to initialize a memory block."
    ...
    if (total_errors == 0)
    {
        printf(
            ...
            "rules.\n");
        /*printf("Execute stall: %ul\nL1V stall: %ul\nL1B stall: %ul\nGS MPRED: %ul\nRETIRED: %ul\n",
                  (uint64_t) hpm_oku(HPM_EXECUTE_STALL),
                  (uint64_t) hpm_oku(HPM_L1V_STALL),
                  (uint64_t) hpm_oku(HPM_L1B_STALL),
                  (uint64_t) hpm_oku(HPM_GS_MPRED),
                  (uint64_t) retired_oku());*/
#if HAS_FLOAT
        if (known_id == 3)
        {
